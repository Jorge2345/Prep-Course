...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  var frase = "";
  for(let i = 0; i<palabras.length-1; i++) frase += palabras[i] + " "
...
function agregarNumeros(numeros) {
  ...
  for (let i=0; i<numeros.length; i++){
    if(typeof numeros[i] == "number" && numeros[i]%1 == 0){
      sumatoria += numeros[i];
    } else return null;
  }
  return sumatoria;
...
function promedioResultadosTest(resultadosTest) {
  ...
  for (let i=0; i<resultadosTest.length; i++){
    if(typeof resultadosTest[i] == "number" && resultadosTest[i]%1 == 0){
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  var contMultiplicador = 1;
  for(let i=0; i<arguments.length; i++){
    contMultiplicador = contMultiplicador * arguments[i];
  }
  return contMultiplicador;
}
...
function empiezaConNueve(n) {
  ...
  /* SIENDO LO MISMO PERO ESCRITO DE OTRA MANERA:
  let num = n.toString()
  if(num.charAt(0) === "9"){
    return true
  }
